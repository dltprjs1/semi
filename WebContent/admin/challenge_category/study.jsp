...
<script type="text/javascript">
	$(function(){
		$("#9").css('color','black');
		$("#1,#3,#4,#5,#6,#7,#8,#2,#10").css('color','lightgray');
	});
</script>
		...
		<nav>
			<ul class="pagination">
				<li class="page-item"><a class="page-link"
					href="study_list.do?page=1">First</a></li>
				<li><a class="page-link"
					href="study_list.do?page=${page - 1 }">Previous</a></li>
				<c:forEach begin="${startBlock }" end="${lastBlock }" var="i">

					<c:if test="${i == page }">
						<li class="page-item active" aria-current="page"><a
							class="page-link" href="study_list.do?page=${i }">${i }</a></li>
					</c:if>

					<c:if test="${i != page }">
						<li class="page-item"><a class="page-link"
							href="study_list.do?page=${i }">${i }</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${lastBlock < allPage }">
					<li class="page-item"><a class="page-link"
						href="study_list.do?page=${page + 1 }">Next</a></li>
					<li class="page-item"><a class="page-link"
						href="study_list.do?page=${allPage }">End</a></li>
				</c:if>
			</ul>
		</nav>
	</div>

...
<script type="text/javascript">
	$(function(){
		$("#3").css('color','black');
		$("#1,#2,#4,#5,#6,#7,#8,#9,#10").css('color','lightgray');
	});
</script>
		...
		<div class="con">
			...
			<div class="all">
				<c:forEach items="${list }" var="dto">
					<div class="training">
						<a class="block"
							href="<%=request.getContextPath()%>/challenge_modify.do?chall_num=${dto.chall_num}&chall_category=식습관&page=${page}"><img
		...
		<nav>
			<ul class="pagination">
				...
				<li><a class="page-link"
					href="eating_list.do?page=${page - 1 }&chall_category=C002">Previous</a></li>
				<c:forEach begin="${startBlock }" end="${lastBlock }" var="i">

					<c:if test="${i == page }">
						<li class="page-item active" aria-current="page"><a
							class="page-link" href="eating_list.do?page=${i }&chall_category=C002">${i }</a></li>
					</c:if>

					<c:if test="${i != page }">
						<li class="page-item"><a class="page-link"
							href="eating_list.do?page=${i }&chall_category=C002">${i }</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${lastBlock < allPage }">
					<li class="page-item"><a class="page-link"
						href="eating_list.do?page=${page + 1 }&chall_category=C002">Next</a></li>
					<li class="page-item"><a class="page-link"
						href="eating_list.do?page=${allPage }&chall_category=C002">End</a></li>
				</c:if>
			</ul>
		</nav>
	</div>

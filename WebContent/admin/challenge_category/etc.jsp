...
<script type="text/javascript">
	$(function(){
		$("#8").css('color','black');
		$("#1,#3,#4,#5,#6,#7,#2,#9,#10").css('color','lightgray');
	});
</script>
		...
		<nav>
			<ul class="pagination">
				<li class="page-item"><a class="page-link"
					href="etc_list.do?page=1&category_code=C007">First</a></li>
				<li><a class="page-link"
					href="etc_list.do?page=${page - 1 }&category_code=C007">Previous</a></li>
				<c:forEach begin="${startBlock }" end="${lastBlock }" var="i">

					<c:if test="${i == page }">
						<li class="page-item active" aria-current="page"><a
							class="page-link" href="etc_list.do?page=${i }&category_code=C007">${i }</a></li>
					</c:if>

					<c:if test="${i != page }">
						<li class="page-item"><a class="page-link"
							href="etc_list.do?page=${i }&category_code=C007">${i }</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${lastBlock < allPage }">
					<li class="page-item"><a class="page-link"
						href="etc_list.do?page=${page + 1 }&category_code=C007">Next</a></li>
					<li class="page-item"><a class="page-link"
						href="etc_list.do?page=${allPage }&category_code=C007">End</a></li>
				</c:if>
			</ul>
		</nav>
	</div>

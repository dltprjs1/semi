...
<script type="text/javascript">
	$(function(){
		$("#5").css('color','black');
		$("#1,#3,#4,#2,#6,#7,#8,#9,#10").css('color','lightgray');
	});
</script>
		...
		<nav>
			<ul class="pagination">
				<li class="page-item"><a class="page-link"
					href="mind_list.do?page=1&category_code=C004">First</a></li>
				<c:if test="${page == 1 }">
		    <li>
		      <a class="page-link" 
		      		href="mind_list.do?page=1&chall_category=C004">Previous</a>
		    </li>
		    </c:if>
		    <c:if test="${page != 1 }">
		    <li>
		      <a class="page-link" 
		      		href="mind_list.do?page=${page - 1 }&chall_category=C004">Previous</a>
		    </li>
		    </c:if>
				<c:forEach begin="${startBlock }" end="${lastBlock }" var="i">
					...
					<c:if test="${i == page }">
						<li class="page-item active" aria-current="page"><a
							class="page-link" href="mind_list.do?page=${i }&category_code=C004">${i }</a></li>
					</c:if>

					<c:if test="${i != page }">
						<li class="page-item"><a class="page-link"
							href="mind_list.do?page=${i }&category_code=C004">${i }</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${lastBlock < allPage }">
					<li class="page-item"><a class="page-link"
						href="mind_list.do?page=${page + 1 }&category_code=C004">Next</a></li>
					<li class="page-item"><a class="page-link"
						href="mind_list.do?page=${allPage }&category_code=C004">End</a></li>
				</c:if>
			</ul>
		</nav>
	</div>
